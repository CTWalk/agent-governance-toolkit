...
[`agt.policies.AcsInterceptor`](../../src/agt/policies/agent_hooks.py) and
crewAI's `use_agent_hooks` context manager.

## The integration in three lines

```python
from agt.policies import AcsInterceptor
...
flowchart LR
    ...
    C -->|SnapshotBuilder| D[AgtRuntime.evaluate]
    D -->|PolicyEvaluation| C
...
- `agt-policies` with the native ACS engine (`agent-control-specification`).
